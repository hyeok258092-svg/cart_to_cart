...
CART_MALL_MAP = {"gmarket": "지마켓", "auction": "옥션", "st11": "11번가", "yes24": "예스24"}


def parse_yes24(html, source):
    """예스24 장바구니 표(HTML)를 직접 저장/캡처해 올린 경우를 파싱한다.
    (북마클릿을 쓰면 CARTMALL 표식이 심긴 옥션식 합성 견적서로 오므로 이 함수를
    거치지 않는다. 이 함수는 수작업으로 tb_cart 표를 캡처해 올린 경우용 백업.)"""
    soup = BeautifulSoup(html, "lxml")
    ...
    for tr in table.find_all("tr"):
        ...
        qty_el = tr.select_one("td.goods_count input")
        total = to_int(total_el.get_text()) if total_el else 0
        qty = to_int(qty_el.get("value")) if qty_el and qty_el.get("value") else 1
